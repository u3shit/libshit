#ifndef GUARD_INDISTINCTIVELY_BOTRYOMYCOTIC_CAP_REFILL_HELLENISES_7812
...
#define LIBSHIT_GEN(name, pre, post, val)                               \
...
#define LIBSHIT_GEN2(name, val) \
    LIBSHIT_GEN(name, , , val)  \
    LIBSHIT_GEN(name, LIBSHIT_NOLUA const, const, val)

    LIBSHIT_GEN2(front, 0)
    LIBSHIT_GEN2(back, vect.size()-1)
#undef LIBSHIT_GEN2
#undef LIBSHIT_GEN

#define LIBSHIT_GEN(dir, typ)                                   \
    ...
    LIBSHIT_GEN(begin,) LIBSHIT_GEN(end,)
    LIBSHIT_GEN(rbegin, reverse_) LIBSHIT_GEN(rend, reverse_)
#undef LIBSHIT_GEN
